...
id로 남긴다. 32가지가 기본으로 들어 있다.

| 갈래 | 기법 |
|---|---|
| 열기 | 개방형 질문, 반영적 경청 |
| 펼치기 | 하루 지도 그리기, 영역별 척도 질문, 영역 간 전이 확인, 기준선 비교, 처방 팔로업, 초점 정하기 |
| 파기 | 트리거 특정, 사고기록, 감정에 이름 붙이기, 생각의 치우침 확인, 증거 검토, 5 Whys, 예외 찾기, 가치 확인, 통제 이분법, 최악·최선·현실, 생각과 거리 두기 |
| 확인 | 요약 후 확인받기, 다른 설명 대보기 |
| 다음 걸음 | 행동 활성화, 최소 단위로 쪼개기, 실행 의도, 문제 해결 단계, 행동 실험, 장애물 먼저 |
| 돌보기 | 좋았던 것 세 가지, 자기자비 |
| 되짚기 | 누적 패턴 관찰, 하루 안에서의 변화, 내 조기 경고 신호 |
...
기록을 브라우저에서 훑어볼 수 있는 화면이 들어 있다. 오른쪽 위 버튼으로 한국어·영어를
전환할 수 있고, 처음 열면 브라우저 언어를 따른다. 내 PC 안에서만 도는 작은 서버가
...
`/api/day/2026-08-27`, `/api/entries/2026-08-27--2130`, `/api/questions`, `/api/methods`, `/api/struggles`, `/api/settings`.
...
│   ├── core.en.yaml
│   ├── methods.yaml    # 상담 기법 32종 (id·출처·근거·설명 문구)
│   ├── references.yaml # 기법별 기대 효과와 더 읽을 출처
│   └── struggles.yaml  # 어려움 지도 (전 세계 성인이 흔히 겪는 정신적 어려움·수치·출처)
├── scripts/howami.py   # 저장소 계층. 읽기·쓰기만 하고 대화는 하지 않는다
├── scripts/serve.py    # 로컬 웹 화면 서버 (127.0.0.1 전용)
├── web/                # 웹 화면 소스 (React + Vite)
└── schema/session.md
```
...
- **v0.2** — 세션 단위 저장(하루 여러 번), 생활 영역별 다관점 분석, 상담 기법 카탈로그(현재 32종),
  해석 검증 단계, `day` 명령 ← 지금
