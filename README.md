...
> "오늘 나, 어때?" — 내 PC 안에서만 돌아가는, AI 에이전트와 함께하는 하루 되짚기

## 이 프로젝트가 우선하는 것

howami는 기능보다 먼저 세 가지를 지키려고 만들었다. 기술적인 선택은 전부 여기서 나온다.

### 1. 기록은 내 것이고, 내 PC를 떠나지 않는다

감정과 정신 상태는 가장 민감한 개인정보다. 그래서 howami에는 서버도, 계정도, 클라우드 동기화도,
텔레메트리도 없다. 기록은 `~/howami/data/` 안의 **마크다운 파일**로 남고, 조회용 SQLite는 그 파일들에서
언제든 다시 만들 수 있는 파생물이다. 웹 화면도 `127.0.0.1`에만 열리는 작은 로컬 서버가 그 폴더를
읽어 보여줄 뿐이며, 외부 폰트나 차트 라이브러리조차 불러오지 않는다. 어디에 무엇이 저장되는지는
화면의 **설정** 탭에서 언제든 확인하고 폴더를 열어 볼 수 있다.

정직하게 밝혀 둘 것이 하나 있다. 대화 자체는 사용자가 쓰는 AI 에이전트(지금은 Claude Code)를 거치므로,
대화 중에 주고받는 말과 에이전트가 참고하려고 읽는 최근 기록은 그 에이전트 제공자의 API로 전송된다.
howami가 보장하는 것은 **저장과 조회가 전부 로컬**이라는 것이고, 대화의 전송 범위는 사용하는
에이전트의 정책을 따른다. 그 부분이 걱정된다면 로컬 모델을 쓰는 에이전트와 함께 쓰는 것이 맞다.

### 2. 사람의 상태를 숫자 하나로 뭉개지 않는다

"오늘 몇 점?"으로 하루를 뭉뚱그리면 아침의 가족과 낮의 일과 밤의 혼자 있는 시간이 하나의 숫자로 뭉개진다.
howami는 **가족·일·혼자 있는 시간·사람들·몸**을 따로 묻고, 그것들이 서로 어떻게 이어졌는지를 본다.
하루에 몇 번을 쓰든 상관없고, 아침에 짧게 저녁에 길게 써도 두 기록이 서로를 안다.

에이전트가 정리한 해석은 언제나 가설이다. 반드시 사용자에게 되돌려 확인받고, 사용자가 고치면
고친 쪽이 기록에 남는다. "자책"이라고 적으려던 것을 사용자가 "창피함"이라고 고치면, 파일에는 창피함이 남는다.
잘했다 못했다를 평가하지 않고, 남과 비교하지도 않는다. 오늘의 나를 있는 그대로 적는 것이 목적이다.

### 3. 정신건강을 다루는 도구답게 신중하다

질문은 감으로 던지지 않는다. 사고기록·감정 명명·증거 검토·예외 찾기·통제 이분법·실행 의도처럼
임상과 연구에서 검증된 상담 기법을 [`questions/methods.yaml`](questions/methods.yaml)에 정리해 두고,
각 항목에 출처(`origin`)와 근거의 성격(`evidence`)을 붙였다. 그날 실제로 쓴 기법만 기록에 남는다.

동시에 howami는 **의료 기기도, 심리 치료도 아니다.** 진단하지 않고 치료를 제안하지 않는다.
낮은 점수가 2주 이상 이어지거나 위기 신호가 느껴지면 전문가와 상담해야 하며,
...
결과는 md 파일 하나로 저장되고, 다음 대화는 그 파일을 읽고 시작하므로 같은 걸 또 묻지 않는다.
...
여기서부터는 기술적인 내용이다. 위의 원칙을 코드로 어떻게 지키는지가 궁금하면 이어서 읽으면 된다.
...
| 탭 | 보여주는 것 |
|---|---|
| **오늘** | 상태 축(에너지·기분·수면·실행)을 최근 7일 평균과 나란히, 생활 영역 점수와 한 줄 메모, 열려 있는 처방, 오늘 세션 본문 |
