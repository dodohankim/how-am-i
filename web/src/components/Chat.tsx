...
export function Chat({ turns }: { turns: Turn[] }) {
  return (
    <div className="chat" role="log" aria-label="대화">
      {turns.map((t, i) => (
        <div className={`bubble ${t.role}`} key={i}>
          <span className="who">{t.role === "agent" ? "howami" : "나"}</span>
...
export function SessionBody({ body }: { body: string }) {
  ...
  return rest ? <Markdown text={rest} /> : <div className="faint small">본문 없음</div>;
}
  ...
  return (
    <>
      <button type="button" className={`chat-btn${has ? "" : " empty"}`} onClick={() => setOpen(true)}
        title={has ? `대화 ${turns.length}턴 보기` : "이 세션에는 대화 기록이 없어요"} aria-label="대화 보기">
        <BubbleIcon />
        대화{has && <b className="num">{turns.length}</b>}
      </button>
...
export function ChatModal({ turns, title, onClose }: { turns: Turn[]; title: string; onClose: () => void }) {
  ...
  return (
    <div className="chat-overlay" onClick={onClose} role="presentation">
      <div className="chat-panel" role="dialog" aria-modal="true" aria-label={`${title} 대화`} onClick={(e) => e.stopPropagation()}>
        <div className="chat-panel-head">
          <span className="num">{title}</span>
          <button type="button" className="chat-close" onClick={onClose} aria-label="닫기">×</button>
        </div>
        <div className="chat-panel-body">
          {turns.length
            ? <Chat turns={turns} />
            : <div className="faint small chat-empty">이 세션에는 대화 기록이 없어요.</div>}
        </div>
      </div>
    </div>
  );
