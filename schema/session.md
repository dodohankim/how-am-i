...
본문 **맨 끝**에 `## 대화` 절을 두면 웹 화면의 오늘·기록 탭에서 세션마다 있는
말풍선 버튼("대화")을 눌렀을 때 그 절만 대화창으로 열린다. 에이전트가 물은 것은 왼쪽,
사용자가 타이핑한 것은 오른쪽이다.

```markdown
## 대화
Q: 어제 정한 "반박하기 전에 되짚어 말하기"는 써 볼 기회가 있었어요?
A: 아직요. 오늘 회사에 결혼 사실을 알렸어요.
Q: 알린 뒤에 지금은 어때요?
A: 살짝 떨리긴 한데 괜찮아요.
  반응을 기다리는 느낌에 가까워요.
```
...
| 표기 | 뜻 | 규칙 |
|---|---|---|
| `Q:` | 에이전트가 물은 것 | 길면 한 줄로 줄여도 된다 |
| `A:` | 사용자가 타이핑한 말 | **그대로 옮긴다.** 줄이거나 다듬거나 보태지 않는다 |
| 그 밖의 줄 | 직전 턴에 이어 붙는다 | 여러 줄 답변을 그대로 남길 때 쓴다 |

`A:`에 사용자가 하지 않은 말을 넣는 것은 기록 무결성 규칙 위반이다. 점수나 영역처럼
질문 세트가 정한 항목을 묻고 답한 턴도 넣는다. 화면은 본문에서 `## 대화` 절을 떼어
내고 요약 절들만 보여주므로, 요약 절들은 그대로 둔다.
`## 대화`가 없는 옛 기록은 버튼이 흐리게 보이고 대화창에 기록이 없다는 한 줄만 나온다.
